        ...
        <div class="col-md-3">
            ...
            <div class="card card-secondarys card-outline">
              <div class="card-body box-profile">
                ...
                <ul class="list-style list-group list-group-unbordered mb-3">
                  <?php

                    $sqlchart="SELECT *,COUNT(m.GENDER) As TotalGender, SUM(CASE WHEN m.GENDER = 'MALE' THEN 1 ELSE 0 END)AS MSEX, SUM(CASE WHEN m.GENDER = 'FEMALE' THEN 1 ELSE 0 END) AS FSEX, b.BRGY_NAME FROM tbl_barangay b LEFT JOIN tbl_members m ON b.BRGY_NAME=m.BARANGAY GROUP BY b.BRGY_NAME ORDER BY TotalGender DESC";
                    $cquery=$conn->query($sqlchart);
                    if($cquery->num_rows >0){
                      while($rowchart=$cquery->fetch_assoc()){
                        $totalMale=$rowchart['MSEX'];
                        $totalFMale=$rowchart['FSEX'];
                        echo ' <li class="list-group-item">
                          '.$rowchart['BRGY_NAME'].' <a class="float-right"><i class="fa fa-male text-green"></i> '.$totalMale.'  <i class="fa fa-female text-pink"></i> '.$totalFMale.'</a>
                          </li>';
                      }
                    }else{
                        echo '<li class="list-group-item">
          ...
          <div class="col-md-9">
            <div class="card">
              <div class="card-header">
               <h3 class="card-title"> 
                 MALE AND FEMALE REPORT PER BARANGAY
                </h3>
              ...
              error_reporting(0);
                  ...
                  $sql = "SELECT *,SUM(CASE WHEN m.GENDER = 'MALE' THEN 1 ELSE 0 END)AS MSEX, SUM(CASE WHEN m.GENDER = 'FEMALE' THEN 1 ELSE 0 END) AS FSEX, m.BARANGAY FROM tbl_members m GROUP BY m.BARANGAY";
                  $query = $db->prepare($sql); 
                  $query->execute();
                  $fetch = $query->fetchAll();
                  foreach ($fetch as $key => $value) {
                    $female[] = array('Title'=>$value['BARANGAY'], 'Female'=>$value['FSEX'], 'Male'=>$value['MSEX']);
                  }
